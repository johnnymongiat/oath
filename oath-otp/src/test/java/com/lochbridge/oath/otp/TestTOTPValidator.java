...
public class TestTOTPValidator {
    ...
    public void isValidShouldSucceedUnderRFC6238TestCases() {
        for (TestVector testVector : RFC6238TestVectors.TEST_VECTORS) {
            assertTrue(TOTPValidator.window(0).isValid(testVector.getKey(), testVector.getTimeStep(), testVector.getDigits(), testVector.getAlgorithm(),
                    testVector.getTotp(), testVector.getTestTime()));
        }
    }
    ...
    public void isValidShouldSucceedWithDefaultWindow() {
        ...
        assertTrue(TOTPValidator.defaultWindow().isValid(RFC6238TestVectors.KEY_FOR_HMAC_SHA_1, source.timeStep(), source.digits(), source.hmacShaAlgorithm(),
                source.value()));
    ...
    public void isValidShouldReturnFalseWithDelayedValidation() {
        ...
        assertFalse(TOTPValidator.defaultWindow().isValid(RFC6238TestVectors.KEY_FOR_HMAC_SHA_1, source.timeStep(), source.digits(), source.hmacShaAlgorithm(),
                source.value(), validationTime));
    }
